-----------------------------------------

@@&&fc_clean_file_name. "moat369_log" "moat369_log_nopath" "PATH"
DEF title = 'File: &&moat369_log_nopath.'
UNDEF moat369_log_nopath

DEF one_spool_text_file = '&&moat369_log.'
DEF one_spool_text_file_rename = 'N'
DEF skip_html = '--'
DEF skip_text_file = ''
@@&&9a_pre_one.

-----------------------------------------

@@&&fc_clean_file_name. "moat369_log2" "moat369_log2_nopath" "PATH"
DEF title = 'File: &&moat369_log2_nopath.'
UNDEF moat369_log2_nopath

DEF one_spool_text_file = '&&moat369_log2.'
DEF one_spool_text_file_rename = 'N'
DEF skip_html = '--'
DEF skip_text_file = ''
@@&&9a_pre_one.

-----------------------------------------

@@&&fc_clean_file_name. "moat369_log3" "moat369_log3_nopath" "PATH"
DEF title = 'File: &&moat369_log3_nopath.'
UNDEF moat369_log3_nopath

DEF one_spool_text_file = '&&moat369_log3.'
DEF one_spool_text_file_rename = 'N'
DEF skip_html = '--'
DEF skip_text_file = ''
@@&&9a_pre_one.

-----------------------------------------

@@&&fc_clean_file_name. "moat369_tkprof" "moat369_tkprof_nopath" "PATH"
DEF title = 'File: &&moat369_tkprof_nopath._sort.txt'
UNDEF moat369_tkprof_nopath

DEF one_spool_text_file = '&&moat369_tkprof._sort.txt'
DEF one_spool_text_file_rename = 'N'
DEF skip_html = '--'
DEF skip_text_file = ''
@@&&skip_tkprof.&&9a_pre_one.

-----------------------------------------

DEF title = 'Shell Execution Log'

DEF one_spool_text_file = '&&orachk_shexec_log.'
DEF one_spool_text_file_rename = 'N'
DEF skip_html = '--'
DEF skip_text_file = ''
@@&&9a_pre_one.

-----------------------------------------

DEF title = 'Version File'

DEF one_spool_text_file = '&&orachk_version_file.'
DEF one_spool_text_file_rename = 'N'
DEF skip_html = '--'
DEF skip_text_file = ''
@@&&9a_pre_one.

-----------------------------------------