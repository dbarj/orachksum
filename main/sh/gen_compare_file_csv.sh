...
then
  ...
  echo "No compare columns for replace. Nothing to do."
  exit 0
fi
...
do
  ...
  ${AWKCMD_CSV} --source '{csv_print_exchange_field_records($0, separator, enclosure, '${v_col_rep}', '${v_col_comp}')}' "${v_infile}" > "${v_outfile}"
